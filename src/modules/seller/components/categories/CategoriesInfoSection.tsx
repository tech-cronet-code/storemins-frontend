...
const isSubcategory = watch("isSubcategory") || type === "SUB";
  ...
  const creatingSub = !categoryId && !!parentCategory; // new subcat flow
...
{isSubcategory && (
  <div className="space-y-1">
    ...
    <input
      ...
      className={`
        w-full rounded-md px-3 py-2 text-sm focus:ring-1 focus:outline-none
        ${creatingSub ? "bg-gray-100 cursor-not-allowed" : "cursor-pointer bg-white"}
        ${errors.category ? "border-red-500 focus:ring-red-500 focus:border-red-500" : "border-gray-300 focus:ring-blue-500 focus:border-blue-500"}
      `}
    />
      ...
      {!isEditParent && (
        <div className="flex items-center space-x-2 pt-1">
          ...
          <label
            ...
            title={isEditSub ? "You are editing a subcategory, this cannot be changed." : ""}
          >
            Add as subcategory
          </label>
        </div>
            ...
            <div className="max-h-[300px] overflow-y-auto divide-y divide-gray-100">
              {filteredCategories.length ? (
                filteredCategories.map((cat) => (
                  <label key={cat.id} className={`flex justify-between items-center px-6 py-4 cursor-pointer transition-colors ${selectedCategoryId === cat.id ? "bg-green-50" : "hover:bg-gray-50"}`}>
