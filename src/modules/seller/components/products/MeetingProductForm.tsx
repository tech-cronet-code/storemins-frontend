...
interface MeetingProductFormProps {
  productId?: string;
}
...
const MeetingProductForm: React.FC<MeetingProductFormProps> = ({
  ...
  const methods = useForm<MeetingProductFormValues>({
    resolver: zodResolver(meetingProductSchema),
    mode: "onChange",
  ...
  const onSubmit = async (data: MeetingProductFormValues) => {
    try {
      ...
      navigate("/seller/catalogue/products/meeting", {
        state: { refresh: true },
      });
    } catch (err: any) {
...
export default MeetingProductForm;
